...
public class MetricsUpdaterThread extends Thread {
	...
	public void run() {
		try {
			Map.Entry<MetricKey, ArrayList<Metric>> singleMetricSequence = MetricCacheSingleton.getInstance().popSingleMetricSequence();
			while (singleMetricSequence != null && running) {
				...
				singleMetricSequence = MetricCacheSingleton.getInstance().popSingleMetricSequence();
			}
		}
		catch (Exception e) {
			e.printStackTrace();
